...
class Car(models.Model):
    ...
    mileage = models.PositiveIntegerField()  # Пробег в км
    color = models.CharField(max_length=50)
    engine_size = models.FloatField()  # Объем двигателя
    transmission = models.CharField(max_length=50)
    availability = models.BooleanField(default=True)  # Доступность
    ...
    image = models.ImageField(upload_to='car_images/', null=True, blank=True)  # Фото автомобиля
    fuel_consumption = models.FloatField(null=True, blank=True)
    history = models.TextField(null=True, blank=True)  # Новое поле для истории

    def save(self, *args, **kwargs):
        if not self.history:  # Назначить случайную историю, если поле пустое
            self.history = random.choice(HISTORIES)
...
class TestDriveRequest(models.Model):
    ...
    date = models.DateField()  # Дата желаемого тест-драйва
    car = models.ForeignKey('Car', on_delete=models.CASCADE)  # Машина для тест-драйва
    test_drive_price = models.DecimalField(max_digits=10, decimal_places=2, default=0)  # Цена тест-драйва
    reservation_made_at = models.DateTimeField(default=timezone.now)  # Когда была сделана бронь
    reservation_cancelled = models.BooleanField(default=False)  # Отменена ли бронь

    def save(self, *args, **kwargs):
        # Перед сохранением рассчитываем стоимость тест-драйва
        if self.car:  # Убедимся, что машина есть
            self.test_drive_price = 5000 + (self.car.engine_size * 1000)
        super().save(*args, **kwargs)

    def check_reservation_expiration(self):
        """
        Проверяем, истекла ли бронь (если прошло больше 1 дня с момента создания).
        Если истекла — отменяем бронь и сохраняем изменения.
        Возвращает True, если бронь была отменена, иначе False.
        """
        if not self.reservation_cancelled and timezone.now() - self.reservation_made_at > timedelta(days=1):
            self.reservation_cancelled = True
            self.save()
            return True
        return False

    def __str__(self):
        return f"Тест-драйв {self.car.make} {self.car.model} для {self.name} на {self.date}"

# Пример списка с историями
HISTORIES = [
